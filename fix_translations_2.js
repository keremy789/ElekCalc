...
const htmlReps = [
    ...
    [
        ...
        '<p class="text-muted" id="verify-text"><span data-i18n="verify_link_sent">Kayıt linkiniz</span> <strong id="verify-email-display"></strong> <span data-i18n="verify_mail_suffix">mail adresinize gönderilmiştir.</span></p>'
    ],
    [
    ...
    [
        ...
        '<button class="btn-text" style="margin-top:0.8rem; display:block; width:100%" id="btn-resend-verify" data-i18n="btn_resend">Tekrar gönder</button>'
    ],
    [
        'Zaten hesabınız var mı?\\n          <button type="button" class="btn-text" id="btn-verify-go-login">Giriş Yap</button>',
        '<span data-i18n="has_account">Zaten hesabınız var mı?</span>\\n          <button type="button" class="btn-text" id="btn-verify-go-login" data-i18n="login">Giriş Yap</button>'
    ],
    [
        'Beni Hatırla\\n          </label>',
    ...
    [
        ...
        '<p style="font-size:0.85rem; margin-bottom:0.6rem;" data-i18n="forgot_pw_sub">E-posta adresinizi girin, şifre sıfırlama linki gönderelim:</p>'
    ],
    [
    ...
    [
        ...
        '<p class="help-text" style="margin-bottom:1rem;" data-i18n="yd_help">Motor akımına göre üç kontaktör ve termik röle akımlarını hesaplar.</p>'
    ],
    [
        '<p class="help-text" style="margin-bottom:1rem;">Akım değerini girin, Panelmaster standartlarına göre uygun\\n            barayı bulun.</p>',
        '<p class="help-text" style="margin-bottom:1rem;" data-i18n="busbar_help">Akım değerini girin, Panelmaster standartlarına göre uygun barayı bulun.</p>'
    ],
    [
...
for (let r of htmlReps) {
    let repFrom = r[0].replace(/\\n/g, "\\n");
    if (html.includes(repFrom)) {
        html = html.split(repFrom).join(r[1].replace(/\\n/g, "\\n"));
    } else {
        console.warn("NOT FOUND IN HTML: ", r[0].substring(0, 30));
    }
}
...
const newKeys = {
    ...
    forgot_pw_sub: { tr: "E-posta adresinizi girin, şifre sıfırlama linki gönderelim:", en: "Enter your email address to receive a password reset link:" },
    btn_send_link: { tr: "Link Gönder", en: "Send Link" },
    yd_help: { tr: "Motor akımına göre üç kontaktör ve termik röle akımlarını hesaplar.", en: "Calculates the currents of three contactors and thermal relay based on motor current." },
    busbar_help: { tr: "Akım değerini girin, Panelmaster standartlarına göre uygun barayı bulun.", en: "Enter the current value to find the suitable busbar according to Panelmaster standards." },
...
code = code.replace('nav_busbar: "Bara"', trKeys + '\\n        nav_busbar: "Bara"');
code = code.replace('nav_busbar: "Busbar"', enKeys + '\\n        nav_busbar: "Busbar"');
...
for (let r of codeReps) {
    if (code.includes(r[0])) {
        code = code.split(r[0]).join(r[1]);
    } else {
        console.warn("NOT FOUND IN JS JS: ", r[0].substring(0, 30));
    }
}
