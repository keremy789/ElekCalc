...
// We check localStorage immediately instead of waiting for window.onload
// so the screen doesn't even flicker to the login page.
const sess = localStorage.getItem('elec_user') || sessionStorage.getItem('elec_user');
if (sess) {
    currentUser = sess;
    // We defer the UI update slightly to allow the DOM to be ready
...
// (Session check moved to top for instant execution)
